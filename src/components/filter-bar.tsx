...
export function FilterBar({
  ...
  const activeCount =
    (state.city ? 1 : 0) + state.genres.length + state.prices.length + (state.when ? 1 : 0);
  ...
  const togglePrice = (price: string) =>
    onChange({
      ...
      prices: state.prices.includes(price)
        ? state.prices.filter((item) => item !== price)
        : [...state.prices, price],
    });
          ...
          <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto py-1">
            {quickChips.map((chip) => (
              <Chip
                key={chip.id}
                active={
                  chip.kind === "when" ? state.when === chip.id : state.genres.includes(chip.id)
                }
        ...
        >
          ...
          <div className="grid gap-6 px-4 pb-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <Chip active={!state.city} onClick={() => onChange({ ...state, city: "" })}>
                  Todas as cidades
                </Chip>
                {cities.map((city) => (
                  <Chip
                    key={city}
                    active={state.city === city}
                    onClick={() => onChange({ ...state, city: state.city === city ? "" : city })}
                  >
                    {city}
                  </Chip>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {priceFilters.map((price) => (
                  <Chip
                    key={price}
                    active={state.prices.includes(price)}
          ...
          <div className="sticky bottom-0 flex items-center gap-3 border-t border-border bg-background p-4">
            <Button
              variant="outline"
              className="gap-2"
              onClick={() => onChange({ ...state, city: "", genres: [], prices: [], when: "" })}
            >
              <X className="size-4" /> Limpar
            </Button>
            <Button className="flex-1" onClick={() => setOpen(false)}>
              Ver {resultCount} eventos
            </Button>
          </div>
        </SheetContent>
      </Sheet>
