  ...
  return (
    <div className="w-full border-b border-white/10 bg-[#150C22] px-4 py-2 text-white">
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-xs font-semibold">
        <span className={`flex items-center gap-1.5 ${gate.online ? "text-emerald-400" : "text-rose-400"}`}>
          <span className={`size-2 rounded-full ${gate.online ? "bg-emerald-400" : "bg-rose-400"}`} />
          {gate.online ? "Online" : "Offline"}
        </span>
        <span className="text-white/70">Última sinc.: {formatTime(gate.lastSync)}</span>
        <span className="text-white/70">
          Check-ins aguardando envio: <span className="font-black text-amber-300">{gate.pending.length}</span>
        </span>
      </div>
      {stale && (
