    ...
    <div className="min-h-screen bg-muted/40">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r-2 border-foreground bg-background p-4 lg:flex">
        <Link to="/produtor" className="mb-6 flex items-center gap-2">
          ...
          <span className="rounded-md bg-primary px-2 py-0.5 text-xs font-bold text-primary-foreground">Produtor</span>
        </Link>
        ...
        <Link to="/" className="mt-4 flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-muted-foreground hover:text-foreground">
          ...
          {onSelectEvent ? (
            <div className="ml-auto w-44 sm:w-60 lg:ml-0">
              <Select value={selectedEvent ?? "todos"} onValueChange={onSelectEvent}>
                <SelectTrigger aria-label="Selecionar evento"><SelectValue placeholder="Todos os eventos" /></SelectTrigger>
                <SelectContent>
        ...
        <main className="mx-auto max-w-6xl px-4 pb-28 pt-6 sm:px-6 lg:pb-12">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              {description ? <p suppressHydrationWarning className="mt-1 text-sm text-muted-foreground">{description}</p> : null}
            </div>
  ...
  return (
    <div
      className={cn(
        ...
        tone === "primary" ? "bg-primary text-primary-foreground" : tone === "sun" ? "bg-sun text-ink" : "bg-background",
      )}
    >
      <p className={cn("text-xs font-bold uppercase tracking-wide", tone === "default" ? "text-muted-foreground" : "opacity-80")}>{label}</p>
      <p className="mt-2 font-display text-2xl font-extrabold">{value}</p>
      {hint ? <p className={cn("mt-1 text-xs", tone === "default" ? "text-muted-foreground" : "opacity-80")}>{hint}</p> : null}
    </div>
  );
}
...
export function PanelCard({ title, action, children, className }: { title?: string; action?: ReactNode; children: ReactNode; className?: string }) {
  return (
    <section className={cn("rounded-2xl border-2 border-foreground bg-background p-4 shadow-pop sm:p-5", className)}>
      {title || action ? (
...
export function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-bold leading-none", tone)}>
      {status}
    </span>
  );
}
...
export function BackToPanel({ to, label }: { to: string; label: string }) {
  return (
    <Button variant="ghost" size="sm" asChild>
      <Link to={to}><ArrowLeft className="size-4" /> {label}</Link>
    </Button>
  );
}
