...
export function BecomeProducer() {
  ...
  const submit = () => {
    ...
    becomeProducer.mutate(name.trim(), {
      ...
      onError: (error) =>
        toast.error(error instanceof Error ? error.message : "Não foi possível criar a produtora."),
    });
  };
    ...
    <div className="grid min-h-screen place-items-center bg-muted/40 px-4">
      <div className="w-full max-w-md rounded-2xl border-2 border-foreground bg-background p-6 text-center shadow-pop">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Sua conta ainda não tem uma produtora. Dê um nome a ela para começar a criar eventos na
          Entrô.
        </p>
        ...
        <Button
          ...
          disabled={!name.trim() || becomeProducer.isPending}
          onClick={submit}
        >
