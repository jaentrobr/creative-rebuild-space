  ...
  head: () => ({
    meta: [
      { title: "Criar evento — Painel Entrô" },
      {
        name: "description",
        content:
          "Monte seu evento em cinco etapas: informações, local, ingressos, taxas e publicação.",
      },
      { property: "og:title", content: "Criar evento — Painel Entrô" },
      {
        property: "og:description",
        content: "Crie lotes, defina taxas e publique seu evento na Entrô.",
      },
...
function NewEvent() {
  ...
  const capacity = types.reduce(
    (sum, t) => sum + t.lots.reduce((s, l) => s + Number(l.quantity || 0), 0),
    0,
  );
  const maxRevenue = types.reduce(
    (sum, t) =>
      sum +
      (t.free ? 0 : t.lots.reduce((s, l) => s + Number(l.quantity || 0) * Number(l.price || 0), 0)),
    0,
  );
  const halfCapacity = types.reduce(
    (sum, t) =>
      t.half
        ? sum + t.lots.reduce((s, l) => s + Math.floor(Number(l.quantity || 0) * 0.4), 0)
        : sum,
    0,
  );
  ...
  const updateLot = (typeId: string, lotId: string, patch: Partial<DraftLot>) =>
    setTypes((current) =>
      current.map((t) =>
        t.id === typeId
          ? { ...t, lots: t.lots.map((l) => (l.id === lotId ? { ...l, ...patch } : l)) }
          : t,
      ),
    );
  ...
  const buildEvent = (status: ProducerEvent["status"]): ProducerEvent => ({
    ...
    salesPaused: false,
    startAt: new Date(
      `${place.startDate || new Date().toISOString().slice(0, 10)}T${place.startTime}`,
    ).toISOString(),
    endAt: new Date(
      `${place.endDate || place.startDate || new Date().toISOString().slice(0, 10)}T${place.endTime}`,
    ).toISOString(),
    gatesAt: new Date(
      `${place.startDate || new Date().toISOString().slice(0, 10)}T${place.gates}`,
    ).toISOString(),
    venue: place.venue,
    cep: place.cep,
        ...
        <PanelCard className="text-center">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            ...
            <Button variant="outline" asChild>
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Bora? ${published.name}: ${eventUrl}`)}`}
                target="_blank"
                rel="noreferrer"
              >
            ...
            <Button variant="ghost" asChild>
              <Link to="/produtor/eventos">Ver meus eventos</Link>
            </Button>
          </div>
        </PanelCard>
      </ProducerLayout>
  ...
  return (
    ...
    >
      ...
      {saved ? (
        <p className="mb-4 rounded-xl bg-sun px-4 py-2 text-sm font-bold text-ink">{saved}</p>
      ) : null}
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Select
                  value={info.genre}
                  onValueChange={(value) => setInfo({ ...info, genre: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  value={info.age}
                  onValueChange={(value) => setInfo({ ...info, age: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <div className="mb-2 flex gap-2">
                {[
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      setInfo((c) => ({ ...c, description: `${c.description}${token}` }))
                    }
                  >
                    {label}
                  </Button>
              ...
              <Textarea
                ...
                placeholder="Conte como vai ser a festa"
              />
            </div>
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-4">
                <img
                  ...
                  className="h-28 w-44 rounded-xl border-2 border-foreground object-cover"
                />
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Prévia usada no card e na página do evento.
              </p>
            </div>
          </div>
        </PanelCard>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setPlace({ ...place, startDate: e.target.value })}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setPlace({ ...place, startTime: e.target.value })}
              />
            </div>
            <div>
              <Label>Término</Label>
              <Input
                type="date"
                value={place.endDate}
                onChange={(e) => setPlace({ ...place, endDate: e.target.value })}
              />
            </div>
            <div>
              ...
              <Input
                type="time"
                value={place.endTime}
                onChange={(e) => setPlace({ ...place, endTime: e.target.value })}
              />
            </div>
            <div>
              ...
              <Input
                type="time"
                value={place.gates}
                onChange={(e) => setPlace({ ...place, gates: e.target.value })}
              />
            </div>
            <div>
      ...
      {step === 2 ? (
        <div className="space-y-4">
              ...
              action={
                types.length > 1 ? (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setTypes((c) => c.filter((t) => t.id !== type.id))}
                  >
            ...
            >
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="flex items-end gap-3">
                  <Switch
                    id={`free-${type.id}`}
                    checked={type.free}
                    onCheckedChange={(value) => updateType(type.id, { free: value })}
                  />
                ...
                <div className="flex items-end gap-3">
                  <Switch
                    id={`half-${type.id}`}
                    checked={type.half}
                    onCheckedChange={(value) => updateType(type.id, { half: value })}
                  />
              ...
              <div className="mt-4 space-y-4">
                {type.lots.map((l) => (
                  <div
                    key={l.id}
                    className="grid gap-3 rounded-xl border border-border p-3 sm:grid-cols-3"
                  >
                    <div>
                      <Label>Nome do lote</Label>
                      <Input
                        value={l.name}
                        onChange={(e) => updateLot(type.id, l.id, { name: e.target.value })}
                      />
                    </div>
                    <div>
                      <Label>Preço (R$)</Label>
                      <Input
                        inputMode="numeric"
                        disabled={type.free}
                        value={type.free ? "0" : l.price}
                        onChange={(e) => updateLot(type.id, l.id, { price: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        inputMode="numeric"
                        value={l.quantity}
                        onChange={(e) => updateLot(type.id, l.id, { quantity: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="date"
                        value={l.startAt}
                        onChange={(e) => updateLot(type.id, l.id, { startAt: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="date"
                        value={l.endAt}
                        onChange={(e) => updateLot(type.id, l.id, { endAt: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        inputMode="numeric"
                        value={l.maxPerOrder}
                        onChange={(e) => updateLot(type.id, l.id, { maxPerOrder: e.target.value })}
                      />
                    </div>
                    <div className="sm:col-span-2">
                      ...
                      <Select
                        value={l.turn}
                        onValueChange={(value) =>
                          updateLot(type.id, l.id, { turn: value as LotTurn })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                        ...
                        <SelectContent>
                          {Object.entries(lotTurnLabels).map(([value, label]) => (
                    ...
                    <div className="flex items-end">
                      {type.lots.length > 1 ? (
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            updateType(type.id, { lots: type.lots.filter((x) => x.id !== l.id) })
                          }
                        >
                          Remover lote
                        </Button>
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => updateType(type.id, { lots: [...type.lots, newLot()] })}
                >
          ...
          <Button variant="outline" onClick={() => setTypes((c) => [...c, newType("Novo tipo")])}>
          ...
          <div
            className={cn(
              ...
              halfOk ? "bg-background" : "bg-sun text-ink",
            )}
          >
            <p className="text-sm font-bold">
              Pela Lei 12.933/2013, a meia-entrada deve ser garantida a no mínimo 40% dos ingressos.
            </p>
            <p className="mt-1 text-sm">
              Hoje você tem <strong>{halfCapacity}</strong> ingressos de meia em{" "}
              <strong>{capacity}</strong> no total (
              {capacity ? Math.round((halfCapacity / capacity) * 100) : 0}%).{" "}
          ...
          <PanelCard title="Resumo">
            <p className="text-sm">
              Capacidade total: <strong>{capacity}</strong> ingressos
            </p>
            <p className="text-sm">
              Receita máxima estimada: <strong>{brl(maxRevenue)}</strong>
            </p>
          </PanelCard>
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          <PanelCard title="Quem paga a taxa">
            <RadioGroup
              ...
              onValueChange={(value) => setSettings({ ...settings, feeMode: value as FeeMode })}
              className="gap-3"
            >
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Pix: 7% (mínimo R$ 3,50). Cartão: 8% (mínimo R$ 3,99).
            </p>
          </PanelCard>

          <PanelCard title="Parcelamento no cartão">
            <Select
              value={String(settings.installments)}
              onValueChange={(value) => setSettings({ ...settings, installments: Number(value) })}
            >
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Juros do parcelamento ficam por conta do comprador.
            </p>
          </PanelCard>
          ...
          <PanelCard title="Cancelamento e transferência">
            <div className="flex items-start gap-3">
              <Switch
                ...
                onCheckedChange={(value) => setSettings({ ...settings, allowCancel: value })}
              />
              <Label htmlFor="cancel" className="leading-snug">
                Permitir cancelamento até 48h antes do evento com taxa de 10% (a taxa fica com
                você), depois dos 7 dias de arrependimento
              </Label>
            </div>
            <div className="mt-4 flex items-center gap-3">
              <Switch
                id="transfer"
                checked={settings.transfer}
                onCheckedChange={(value) => setSettings({ ...settings, transfer: value })}
              />
            ...
            {settings.transfer ? (
              <div className="mt-3 w-56">
                ...
                <Select
                  value={String(settings.transferBlockHours)}
                  onValueChange={(value) =>
                    setSettings({ ...settings, transferBlockHours: Number(value) })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
          ...
          <PanelCard title="Visibilidade">
            <RadioGroup
              ...
              onValueChange={(value) =>
                setSettings({ ...settings, visibility: value as Visibility })
              }
              className="gap-3"
            >
              <label className="flex items-center gap-3 rounded-xl border border-border p-3 text-sm font-semibold">
                <RadioGroupItem value="publico" /> Público (aparece na busca)
              </label>
              <label className="flex items-center gap-3 rounded-xl border border-border p-3 text-sm font-semibold">
                <RadioGroupItem value="privado" /> Privado (só por link)
              </label>
            </RadioGroup>
          </PanelCard>
        </div>
        ...
        <div className="space-y-4">
          {[
            ...
            },
            {
              ...
              to: 1,
              rows: [
                `Início: ${place.startDate || "—"} ${place.startTime}`,
                `Local: ${place.venue || "—"}`,
                `Endereço: ${place.address} ${place.number}, ${place.city}`,
              ],
            },
            {
              title: "Ingressos",
              ...
              rows: [
                `${types.length} tipos`,
                `Capacidade: ${capacity}`,
                `Receita máxima: ${brl(maxRevenue)}`,
                `Meia-entrada: ${halfOk ? "atende aos 40%" : "abaixo dos 40%"}`,
              ],
            },
            {
              title: "Configurações",
              to: 3,
              rows: [
                settings.feeMode === "repassar"
                  ? "Taxa repassada ao comprador"
                  : "Taxa absorvida por você",
                `Parcelamento até ${settings.installments}x`,
                settings.visibility === "publico" ? "Público" : "Privado",
              ],
            },
          ].map((block) => (
            <PanelCard
              ...
              title={block.title}
              action={
                <Button size="sm" variant="ghost" onClick={() => setStep(block.to)}>
                  Editar
                </Button>
              }
            >
      ...
      <div className="mt-6 flex justify-between gap-3">
        ...
        {step < 4 ? (
          <Button onClick={() => setStep((s) => Math.min(4, s + 1))}>Continuar</Button>
        ) : null}
      </div>
      ...
      <Dialog open={termsModal} onOpenChange={setTermsModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Antes de publicar seu primeiro evento, confirme que você está de acordo com os pontos
              abaixo.
            </DialogDescription>
          </DialogHeader>
          <ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground">
            ...
            <li>
              Depois de pedir adiantamento, não é possível cancelar o evento sem devolver os
              valores.
            </li>
          </ul>
          <a
            href="/termos-produtor"
            target="_blank"
            rel="noreferrer"
          ...
          <DialogFooter>
            <Button disabled={!termsChecked} onClick={acceptAndPublish}>
              Aceitar e publicar
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <AlertTriangle className="size-5 text-primary" /> Verificação obrigatória
            </DialogTitle>
            <DialogDescription>
              Para publicar eventos com ingresso pago, sua conta precisa estar verificada. É o que
              garante que o dinheiro das vendas caia no seu CPF ou CNPJ. Eventos só gratuitos
              publicam sem verificação.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              onClick={() => navigate({ to: "/produtor/verificacao", search: { voltar: "novo" } })}
            >
