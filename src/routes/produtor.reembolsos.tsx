...
export const Route = createFileRoute("/produtor/reembolsos")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Regras aplicadas, valores devolvidos e status de cada pedido." },
...
function ProducerRefunds() {
  ...
  const list = refunds.filter(
    (refund) => (eventId === "todos" || refund.eventId === eventId) && (status === "todos" || refund.status === status),
  );

  return (
    <ProducerLayout title="Reembolsos" description="Os reembolsos seguem as regras da Entrô e acontecem automaticamente. Aqui você só acompanha.">
      {frozenBalance > 0 ? (
        <div className="mb-4 rounded-2xl border-2 border-foreground bg-sun p-4 text-ink shadow-pop">
          <p className="font-display text-base font-extrabold">Dinheiro congelado por evento cancelado</p>
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <div className="w-56">
          <Select value={eventId} onValueChange={setEventId}>
            <SelectTrigger aria-label="Filtrar por evento"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-3">
          {list.map((refund) => (
            <div key={refund.id} className="rounded-xl border border-border p-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">{eventName(refund.eventId)} · {refund.ticket}</p>
              <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-4">
                <p>Compra: <span className="font-semibold text-foreground">{shortDate(refund.purchasedAt)}</span></p>
                <p>Pedido: <span className="font-semibold text-foreground">{shortDate(refund.requestedAt)}</span></p>
                <p>Regra: <span className="font-semibold text-foreground">{refund.rule}</span></p>
                <p>Devolvido: <span className="font-semibold text-foreground">{brl(refund.amount)}</span></p>
              </div>
              <p className="mt-2 text-xs text-muted-foreground">Motivo: {refund.reason}</p>
            </div>
          ))}
          {list.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum reembolso com esses filtros.</p> : null}
        </div>
      </PanelCard>
    </ProducerLayout>
  );
