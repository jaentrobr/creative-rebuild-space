...
export const Route = createFileRoute("/produtor/verificacao")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Envie seus dados e documentos para receber o dinheiro das vendas.",
      },
      { property: "og:title", content: "Verificação da conta — Painel Entrô" },
      {
        property: "og:description",
        content: "Pessoa física ou jurídica, documentos e status da análise.",
      },
...
function Verification() {
  ...
  const docFields =
    personType === "juridica"
      ? [
          ["frente", "Documento com foto (frente)"],
          ["verso", "Documento com foto (verso)"],
          ["selfie", "Selfie segurando o documento"],
          ["contrato", "Contrato social ou CCMEI"],
        ]
      : [
          ["frente", "Documento com foto (frente)"],
          ["verso", "Documento com foto (verso)"],
  ...
  const validateData = () => {
    if (personType === "fisica") {
      ...
      if (pj.cnpj.replace(/\D/g, "").length !== 14 || !pj.legalName)
        return "Confira o CNPJ e a razão social.";
      if (!validateCpf(pj.ownerCpf) || !adult(pj.ownerBirth))
        return "Confira os dados do responsável (maior de 18 anos).";
    }
  ...
  const statusIcon =
    verification === "Aprovado" ? BadgeCheck : verification === "Recusado" ? XCircle : Clock;
  const StatusIcon = statusIcon;

  return (
    <ProducerLayout
      title="Verificação"
      description="Pedimos isso por exigência do Banco Central. É o que garante que o dinheiro das suas vendas cai no seu CPF ou CNPJ."
    >
      ...
      {step === 0 ? (
        <PanelCard title="Tipo de conta">
          <RadioGroup
            value={personType}
            onValueChange={(value) => setPersonType(value as "fisica" | "juridica")}
            className="gap-3"
          >
            <label className="flex items-center gap-3 rounded-xl border border-border p-3 text-sm font-semibold">
              <RadioGroupItem value="fisica" /> Pessoa física
            </label>
            <label className="flex items-center gap-3 rounded-xl border border-border p-3 text-sm font-semibold">
              <RadioGroupItem value="juridica" /> Pessoa jurídica
            </label>
          </RadioGroup>
          <p className="mt-3 rounded-xl bg-muted p-3 text-sm text-muted-foreground">
            Pessoa jurídica tem limite maior de saque via Pix e pode usar antecipação automática.
          </p>
        ...
        <PanelCard title={personType === "fisica" ? "Seus dados" : "Dados da empresa"}>
          {personType === "fisica" ? (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  value={pf.cpf}
                  onChange={(e) => setPf({ ...pf, cpf: maskCpf(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  value={pf.birth}
                  onChange={(e) => setPf({ ...pf, birth: maskDate(e.target.value) })}
              ...
              <div>
                ...
                <Input
                  value={pf.income}
                  onChange={(e) => setPf({ ...pf, income: e.target.value })}
                  placeholder="R$ 5.000"
                />
              </div>
              ...
              <div>
                <Label>CEP</Label>
                <Input value={pf.cep} onChange={(e) => setPf({ ...pf, cep: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  value={pf.number}
                  onChange={(e) => setPf({ ...pf, number: e.target.value })}
                />
              </div>
              <div>
                <Label>Cidade</Label>
              ...
              <div>
                <Label>Estado</Label>
                <Input value={pf.state} onChange={(e) => setPf({ ...pf, state: e.target.value })} />
              </div>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  value={pj.legalName}
                  onChange={(e) => setPj({ ...pj, legalName: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  value={pj.tradeName}
                  onChange={(e) => setPj({ ...pj, tradeName: e.target.value })}
                />
              </div>
              <div>
                <Label>Tipo de empresa</Label>
                <Select
                  value={pj.companyType}
                  onValueChange={(value) => setPj({ ...pj, companyType: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Input
                  value={pj.revenue}
                  onChange={(e) => setPj({ ...pj, revenue: e.target.value })}
                  placeholder="R$ 50.000"
                />
              </div>
              <div>
                <Label>CEP da empresa</Label>
                <Input value={pj.cep} onChange={(e) => setPj({ ...pj, cep: e.target.value })} />
              </div>
              <div>
                <Label>Endereço</Label>
                <Input
                  value={pj.address}
                  onChange={(e) => setPj({ ...pj, address: e.target.value })}
                />
              </div>
              <div>
                <Label>Número</Label>
                <Input
                  value={pj.number}
                  onChange={(e) => setPj({ ...pj, number: e.target.value })}
                />
              </div>
              <div>
                <Label>Cidade</Label>
                <Input value={pj.city} onChange={(e) => setPj({ ...pj, city: e.target.value })} />
              </div>
              <div>
                <Label>Estado</Label>
                <Input value={pj.state} onChange={(e) => setPj({ ...pj, state: e.target.value })} />
              </div>
              <div className="sm:col-span-2">
                <Label>Responsável — nome completo</Label>
                <Input
                  value={pj.ownerName}
                  onChange={(e) => setPj({ ...pj, ownerName: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  value={pj.ownerCpf}
                  onChange={(e) => setPj({ ...pj, ownerCpf: maskCpf(e.target.value) })}
                />
              </div>
              <div>
                <Label>Responsável — nascimento</Label>
                <Input
                  value={pj.ownerBirth}
                  onChange={(e) => setPj({ ...pj, ownerBirth: maskDate(e.target.value) })}
                  placeholder="00/00/0000"
                />
              </div>
            </div>
          ...
          <div className="mt-4 max-w-sm">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Usado apenas para contato sobre sua conta.
            </p>
          </div>
        ...
        <PanelCard title="Documentos">
          <div className="grid gap-4 sm:grid-cols-2">
            {docFields.map(([key, label]) => (
              <label
                key={key}
                className="cursor-pointer rounded-xl border-2 border-dashed border-border p-4 text-center"
              >
                {docs[key!] ? (
                  <img
                    src={docs[key!]}
          ...
          <p className="mt-4 flex items-start gap-2 rounded-xl bg-muted p-3 text-sm text-muted-foreground">
            ...
            Pedimos isso por exigência do Banco Central. É o que garante que o dinheiro das suas
            vendas cai no seu CPF ou CNPJ.
          </p>
          <div className="mt-4 flex gap-2">
            <Button variant="outline" onClick={() => setStep(1)}>
              Voltar
            </Button>
            <Button onClick={() => void submit()} disabled={savePrivate.isPending}>
              Enviar para análise
            </Button>
          </div>
        </PanelCard>
      ...
      {step === 3 ? (
        <PanelCard title="Status da verificação">
          ...
          {voltar === "novo" && verification === "Aprovado" ? (
            <Button
              className="mt-4"
              onClick={() => navigate({ to: "/produtor/eventos/novo", search: { editar: "" } })}
            >
              Voltar para a criação do evento
            </Button>
          ) : null}
