...
export const Route = createFileRoute("/admin/financeiro")({
  head: () => ({
    meta: [{ title: "Financeiro — Admin Entrô" }, { name: "robots", content: "noindex, nofollow" }],
  }),
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const [ordersRes, settingsRes, advancesRes, payoutsRes] = await Promise.all([
        db
          .from("orders")
          .select("total, service_fee, payment_method, paid_at")
          .eq("status", "paid")
      ...
      const advances = (advancesRes.data ?? []) as unknown as (Tables<"advances"> & {
        events: { title: string } | null;
      })[];
      const payouts = payoutsRes.data ?? [];
      const producerIds = Array.from(
        new Set([...advances.map((a) => a.producer_id), ...payouts.map((p) => p.producer_id)]),
      );
      const producersRes =
...
function AdminFinance() {
  ...
  return (
    ...
    >
      ...
      {isError ? (
        <ErrorState
          description="Não conseguimos carregar os dados financeiros."
          onRetry={() => refetch()}
        />
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <StatCard
              label="Receita bruta (volume pago)"
              value={brl(stats.grossRevenue)}
              tone="primary"
            />
          ...
          <PanelCard title="Receita bruta por dia" className="mt-5">
            {stats.chartData.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">Nenhuma venda paga nesse período.</p>
            )}
          </PanelCard>
          ...
          <PanelCard title="Receita de taxas por forma de pagamento" className="mt-5">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Taxa Pix</span>
                <strong>{brl(stats.pixFeeRevenue)}</strong>
              </div>
              <div className="flex justify-between">
                <span>Taxa cartão</span>
                <strong>{brl(stats.cardFeeRevenue)}</strong>
              </div>
            </div>
          </PanelCard>
          ...
          {data.settings ? (
            <PanelCard title="Política de retenção atual" className="mt-5">
              <p className="text-sm text-muted-foreground">
                {(data.settings.retention_percent * 100).toString().replace(".", ",")}% do valor de
                cada evento fica retido por {data.settings.retention_days} dia(s) após o repasse,
                conforme configurações da plataforma. O saldo consolidado da conta Entrô depende da
                integração com o gateway de pagamento (fora do escopo deste painel).
              </p>
            </PanelCard>
          ) : null}
          ...
          <PanelCard title="Adiantamentos e antecipações" className="mt-5">
            <div className="space-y-2 text-sm">
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {a.events?.title ?? "—"} · Solicitado em {shortDate(a.created_at)}
                    </p>
                  </div>
              ...
              {data.advances.length === 0 ? (
                <p className="text-muted-foreground">Nenhum adiantamento registrado.</p>
              ) : null}
            </div>
          </PanelCard>
            ...
            <div className="divide-y divide-border text-sm">
              {data.payouts.map((p) => (
                <div key={p.id} className="flex flex-wrap justify-between gap-2 py-2">
                  <span>
                    {data.producerNames.get(p.producer_id) ?? "—"} · {p.method.toUpperCase()} ·{" "}
              ...
              {data.payouts.length === 0 ? (
                <p className="py-2 text-muted-foreground">Nenhum repasse registrado.</p>
              ) : null}
            </div>
          </PanelCard>
        </>
