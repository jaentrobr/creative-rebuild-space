...
export const Route = createFileRoute("/produtor/configuracoes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Edite os dados da sua produtora, chave Pix e notificações.",
      },
      { property: "og:title", content: "Configurações da produtora — Painel Entrô" },
      {
        property: "og:description",
        content: "Perfil público, chave Pix de saque e avisos por evento.",
      },
...
function ProducerSettings() {
  ...
  return (
    <ProducerLayout
      title="Configurações"
      description="Tudo o que aparece na página dos seus eventos e onde o dinheiro cai."
    >
      <div className="grid gap-4 lg:grid-cols-2">
        <PanelCard title="Dados da produtora">
          <div className="grid gap-4">
            <div className="flex items-center gap-3">
              <div className="grid size-14 place-items-center rounded-full bg-primary font-display text-lg font-extrabold text-primary-foreground">
                {form.logoInitials}
              </div>
            ...
            <div>
              <Label>Descrição (aparece na página do evento)</Label>
              <Textarea
                rows={4}
                value={form.bio}
        ...
        <div className="space-y-4">
          <PanelCard title="Chave Pix para saque">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Só aceitamos chaves no mesmo CPF ou CNPJ da conta verificada.
            </p>
          </PanelCard>
          ...
          <PanelCard title="Termos do produtor">
            {termsAcceptance ? (
              <p className="text-sm text-muted-foreground">
                Termos aceitos: versão {termsAcceptance.version} em{" "}
                {new Date(termsAcceptance.acceptedAt).toLocaleDateString("pt-BR")} às{" "}
                {new Date(termsAcceptance.acceptedAt).toLocaleTimeString("pt-BR", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </p>
              ...
              <p className="text-sm text-muted-foreground">
                Você ainda não aceitou os Termos do produtor (versão {EMPRESA.VERSAO_TERMOS}).
              </p>
            )}
            <Link
              to="/termos-produtor"
              ...
              >
                ...
                <Switch
                  ...
                  onCheckedChange={(value) =>
                    producerActions.updateNotifications({
                      [key as keyof typeof notifications]: value,
                    })
                  }
                />
              </div>
      ...
      <div className="mt-5 flex items-center gap-3">
        <Button
          onClick={() => {
            producerActions.updateProfile(form);
            setSaved(true);
