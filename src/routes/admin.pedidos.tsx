...
import { ORDER_STATUS_LABELS, PAYMENT_METHOD_LABELS, PROCESS_STATUS_LABELS, fetchProfilesMap, logAudit } from "@/lib/admin-store";
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [ticketsRes, refundsRes] = await Promise.all([
        db.from("tickets").select("*").eq("order_id", orderId).order("created_at", { ascending: true }),
        db.from("refunds").select("*").eq("order_id", orderId).order("created_at", { ascending: false }),
      ]);
...
function AdminOrders() {
  ...
  if (selectedOrder) {
    return <OrderDetail order={selectedOrder} buyer={selectedProfile} onBack={() => setSelected(null)} />;
  }

  return (
    <AdminLayout title="Pedidos" description="Busque por código, e-mail ou CPF do comprador.">
      <div className="mb-4">
        <Input placeholder="Buscar por código, e-mail ou CPF" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full sm:w-96" />
      </div>
      {isError ? (
            ...
            {filtered.map((o) => {
              ...
              return (
                <button key={o.id} onClick={() => setSelected(o.id)} className="flex w-full flex-wrap items-center justify-between gap-2 py-3 text-left">
                  <div>
                    <p className="font-display text-sm font-extrabold">{o.code} · {p?.full_name ?? "Comprador"}</p>
                    <p className="text-xs text-muted-foreground">{p?.email ?? "—"} · {o.events?.title ?? "—"} · {shortDateTime(o.created_at)}</p>
                  </div>
...
function OrderDetail({ order, buyer, onBack }: { order: OrderRow; buyer: Profile | null; onBack: () => void }) {
  const { user } = useAuth();
  ...
  const requestRefund = async () => {
    try {
      ...
      if (error) throw error;
      await logAudit({ actorId: user?.id ?? null, action: "request_refund", entity: "orders", entityId: order.id, details: { reason: reason.trim(), amount: order.total } });
      toast.success("Estorno solicitado. O processamento efetivo depende da integração com o gateway de pagamento.");
      setReason("");
      setRefundOpen(false);
    ...
    <AdminLayout
      ...
      actions={
        <>
          ...
          <Button size="sm" variant="outline" disabled title="Envio de e-mail depende de um serviço de notificações que não está disponível neste painel.">
            Reenviar ingressos por e-mail
          </Button>
          {canRequestRefund ? (
            <Button size="sm" variant="destructive" onClick={() => setRefundOpen(true)}>Solicitar estorno</Button>
          ) : null}
        </>
      }
    >
      <div className="grid gap-4 lg:grid-cols-2">
        <PanelCard title="Comprador">
          <div className="space-y-1 text-sm">
            <p>Nome: <span className="font-semibold">{buyer?.full_name ?? "—"}</span></p>
        ...
        <PanelCard title="Pagamento">
          <div className="space-y-1 text-sm">
            <p>Forma de pagamento: <span className="font-semibold">{PAYMENT_METHOD_LABELS[order.payment_method]}{order.payment_method === "credit_card" ? ` · ${order.installments}x` : ""}</span></p>
            <p>Subtotal: <span className="font-semibold">{brl(order.subtotal)}</span></p>
            <p>Desconto: <span className="font-semibold">{brl(order.discount)}</span></p>
            <p>Taxa de serviço: <span className="font-semibold">{brl(order.service_fee)}</span></p>
            <p>Total: <span className="font-semibold">{brl(order.total)}</span></p>
            <p>Status: <StatusPill status={order.status} /></p>
            <p>Pago em: <span className="font-semibold">{order.paid_at ? shortDateTime(order.paid_at) : "—"}</span></p>
          </div>
        </PanelCard>
      </div>
      ...
      <PanelCard title="Ingressos" className="mt-5">
        {detail.isError ? (
          <ErrorState description="Não conseguimos carregar os ingressos." onRetry={() => detail.refetch()} />
        ) : detail.isLoading ? (
          ...
          <div className="divide-y divide-border text-sm">
            {(detail.data?.tickets ?? []).map((t) => (
              <div key={t.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                <span>{t.holder_name} · {t.holder_email ?? "—"}</span>
                <StatusPill status={t.status} />
              </div>
            ))}
            {(detail.data?.tickets ?? []).length === 0 ? <p className="py-2 text-muted-foreground">Nenhum ingresso neste pedido.</p> : null}
          </div>
        )}
      </PanelCard>
      ...
      <PanelCard title="Reembolsos solicitados" className="mt-5">
        {detail.isLoading ? (
          ...
          <div className="divide-y divide-border text-sm">
            ...
            {(detail.data?.refunds ?? []).length === 0 ? <p className="py-2 text-muted-foreground">Nenhum estorno solicitado para este pedido.</p> : null}
          </div>
        )}
      </PanelCard>
      ...
      <AlertDialog open={refundOpen} onOpenChange={setRefundOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Isso registra um pedido de estorno ({PROCESS_STATUS_LABELS.requested}). A efetivação do reembolso ao comprador depende do gateway de pagamento e não é feita por este painel.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction disabled={!reason.trim()} onClick={(e) => { e.preventDefault(); requestRefund(); }}>
