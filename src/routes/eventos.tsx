...
export const Route = createFileRoute("/eventos")({
  validateSearch: (search) =>
    z.object({ q: z.string().catch(""), cidade: z.string().catch(""), genero: z.string().catch(""), quando: z.string().catch("") }).parse(search),
  head: () => ({
    meta: [
  ...
  return (
    <>
      <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6">
        <h1 className="mb-2 text-4xl font-bold sm:text-5xl">{q ? `Resultados para “${q}”` : "Todos os eventos"}</h1>
        <p className="mb-6 text-muted-foreground">Escolha os filtros e encontre a combinação perfeita.</p>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        {error ? (
          ...
          <>
            <p className="mb-4 text-sm font-semibold text-muted-foreground">{results.length} eventos encontrados</p>
            <div className="grid grid-cols-1 gap-3 sm:hidden">
              {results.slice(0, visible).map((event) => <EventCardRow key={event.slug} event={event} />)}
            </div>
            <div className="hidden gap-5 sm:grid sm:grid-cols-2 lg:grid-cols-3">
              {results.slice(0, visible).map((event) => <EventCard key={event.slug} event={event} />)}
            </div>
            {visible < results.length && (
              <div className="mt-8 text-center">
                <Button variant="outline" onClick={() => setVisible((current) => current + 10)}>Carregar mais</Button>
              </div>
            )}
          </>
        ) : (
          <div className="my-16 text-center">
            <div className="mx-auto grid size-20 place-items-center rounded-full bg-secondary text-4xl">✦</div>
