...
function TicketRow({ ticket, needsChoice }: { ticket: TicketWithEvent; needsChoice?: boolean }) {
  ...
  return (
    <Link to="/meus-ingressos/$id" params={{ id: ticket.id }} className="flex gap-4 rounded-xl border border-border bg-card p-3 transition-colors hover:border-primary">
      <img src={event.banner_url ?? "/placeholder.svg"} alt={event.title} loading="lazy" width={200} height={200} className="size-24 shrink-0 rounded-lg object-cover" />
      <div className="min-w-0 flex-1">
        ...
        <p className="flex items-center gap-1 truncate text-sm text-muted-foreground"><MapPin className="size-3.5" />{event.venue_name ?? ""}{event.city ? ` · ${event.city}` : ""}</p>
        <p className="mt-1 text-sm font-semibold">{ticket.ticket_types?.name ?? "Ingresso"} · {ticket.lots?.name ?? ""} · {ticket.holder_name}</p>
      </div>
    </Link>
  );
}
