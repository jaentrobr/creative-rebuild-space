  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (verification !== "todas") {
        rows = rows.filter((r) => (r.producer_private?.verification_status ?? "not_started") === verification);
      }
  ...
  return (
    <AdminLayout title="Produtores" description="Todos os produtores cadastrados na Entrô.">
      <div className="mb-4 flex flex-wrap gap-3">
        <Input placeholder="Buscar por nome" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="w-full sm:w-72" />
        <div className="w-48">
          <Select value={verification} onValueChange={(v) => { setVerification(v); setPage(0); }}>
      ...
      {isError ? (
        <ErrorState description="Não conseguimos carregar os produtores." onRetry={() => refetch()} />
      ) : isLoading || !data ? (
        ...
        <PanelCard>
          <div className="space-y-3">
            {data.rows.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => setSelected(p.id)} className="w-full rounded-xl border border-border p-4 text-left transition-colors hover:border-foreground">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-display text-base font-extrabold">{p.display_name}</p>
                    <div className="flex flex-wrap gap-1">
                      {p.producer_private?.is_blocked ? <span className="rounded-full bg-destructive/15 px-3 py-1 text-xs font-bold text-destructive">Bloqueado</span> : null}
                      {p.producer_private?.risk_flag ? <span className="rounded-full bg-sun px-3 py-1 text-xs font-bold text-ink">Risco</span> : null}
                      <StatusPill status={VERIFICATION_LABELS[verif]} />
                    </div>
                  </div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {p.producer_private?.document ?? "Sem documento"} · Cadastro: {shortDate(p.created_at)}
                  </p>
                </button>
              );
            })}
            {data.rows.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum produtor por aqui ainda.</p> : null}
          </div>
          <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
            <span>Página {page + 1} de {Math.max(1, Math.ceil(data.count / PAGE_SIZE))}</span>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" disabled={page === 0} onClick={() => setPage((p) => p - 1)}>Anterior</Button>
              <Button size="sm" variant="outline" disabled={(page + 1) * PAGE_SIZE >= data.count} onClick={() => setPage((p) => p + 1)}>Próxima</Button>
            </div>
          </div>
        </PanelCard>
      )}
...
function ProducerDetail({ producerId, onBack }: { producerId: string; onBack: () => void }) {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const [producerRes, eventsRes, ordersAggRes] = await Promise.all([
        ...
        db.from("events").select("id, title, status, is_featured").eq("producer_id", producerId).order("created_at", { ascending: false }),
      ...
      if (eventIds.length > 0) {
        const { data: orders, error } = await db.from("orders").select("total, event_id").eq("status", "paid").in("event_id", eventIds);
        if (error) throw error;
        volume = (orders ?? []).reduce((s, o) => s + Number(o.total), 0);
      }
      return {
        producer: producerRes.data as unknown as (Tables<"producers"> & { producer_private: Tables<"producer_private"> | null }) | null,
  ...
  const setVerification = async (status: Enums<"verification_status">) => {
    setBusy(true);
    try {
      const { error } = await db.from("producer_private").update({ verification_status: status, verification_notes: note || null }).eq("producer_id", producerId);
      if (error) throw error;
      await logAudit({
        actorId: user?.id ?? null,
        action: status === "approved" ? "approve_producer_verification" : "reject_producer_verification",
  ...
  if (isError) return <AdminLayout title="Produtor" actions={<Button size="sm" variant="outline" onClick={onBack}>Voltar</Button>}><ErrorState onRetry={() => refetch()} /></AdminLayout>;
  if (isLoading || !data) return <AdminLayout title="Carregando…"><Skeleton className="h-64 w-full" /></AdminLayout>;

  const producer = data.producer;
  if (!producer) return <AdminLayout title="Produtor não encontrado" actions={<Button size="sm" variant="outline" onClick={onBack}>Voltar</Button>}><p className="text-muted-foreground">Este produtor não existe mais.</p></AdminLayout>;
  const verif = producer.producer_private?.verification_status ?? "not_started";

  return (
    <AdminLayout title={producer.display_name} description={producer.producer_private?.legal_name ?? undefined} actions={<Button size="sm" variant="outline" onClick={onBack}>Voltar</Button>}>
      ...
      <PanelCard title="Dados cadastrais" className="mt-5">
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          <p>Documento: <span className="font-semibold">{producer.producer_private?.document ?? "—"}</span></p>
      ...
      <PanelCard title="Verificação de identidade" className="mt-5">
        <p className="text-sm">Status atual: <StatusPill status={VERIFICATION_LABELS[verif]} /></p>
        {producer.producer_private?.verification_notes ? (
          <p className="mt-2 text-sm text-muted-foreground">Última observação: {producer.producer_private.verification_notes}</p>
        ) : null}
        <div className="mt-3">
          <Textarea placeholder="Observação (opcional)" value={note} onChange={(e) => setNote(e.target.value)} />
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          <Button size="sm" disabled={busy} onClick={() => setVerification("approved")}>Aprovar verificação</Button>
          <Button size="sm" variant="destructive" disabled={busy} onClick={() => setVerification("rejected")}>Recusar verificação</Button>
        </div>
        <p className="mt-3 text-xs text-muted-foreground">
          Dados bancários/Asaas (conta e carteira) não são editáveis por aqui — dependem da integração de pagamento, fora de escopo.
        </p>
      </PanelCard>
        ...
        <div className="divide-y divide-border text-sm">
          {data.events.map((e) => (
            <div key={e.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
              <span>{e.title}{e.is_featured ? " ⭐" : ""}</span>
              <StatusPill status={e.status} />
            </div>
          ))}
          {data.events.length === 0 ? <p className="text-muted-foreground">Nenhum evento cadastrado.</p> : null}
        </div>
      </PanelCard>
    </AdminLayout>
  );
